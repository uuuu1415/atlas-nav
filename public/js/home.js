...
function linkMatches(link) {
  const searchable =
    `${link.name} ${link.description} ${link.aliases || ''} ${link.url}`.toLowerCase();
...
function createLinkCard(link) {
  ...
  anchor.querySelector('small').textContent =
...
function recordRecentLink(link) {
  const existing = JSON.parse(
    localStorage.getItem('atlas-recent-links') || '[]',
...
function applyDesignSettings(settings) {
  ...
  document.body.dataset.layout =
    localStorage.getItem('atlas-layout') || settings.layout || 'standard';
}
...
function renderSearchMenu() {
  ...
  menu.innerHTML =
    `<button data-search-mode="local"><span class="menu-dot"></span>${text(state.language, 'local')}</button>` +
    engines
      .map(
        (engine) =>
  ...
  menu.querySelectorAll('[data-search-mode]').forEach((button) => {
    button.addEventListener('click', () => {
      state.searchMode =
        button.dataset.searchMode === 'local'
          ? 'local'
          : Number(button.dataset.searchMode);
      state.query = '';
      $('#search').value = '';
      menu.hidden = true;
      render();
...
function renderSections(categories, pinned) {
  ...
  const recentIds = JSON.parse(localStorage.getItem('atlas-recent-links') || '[]').map(
    (item) => item.id,
  );
...
function render() {
  ...
  $('#search').placeholder =
    state.searchMode === 'local'
      ? text(state.language, 'search')
      : text(state.language, 'external');
  document.querySelectorAll('#layout-menu [data-layout]').forEach((button) => {
    button.textContent = text(state.language, button.dataset.layout);
  });
  ...
  $('#search-mode-label').textContent =
    state.searchMode === 'local'
      ? text(state.language, 'local')
      : engine?.name || 'Search';
  $('#result-note').innerHTML =
    state.searchMode === 'local'
      ? state.query
        ? `${resultCount} results · <b>↵</b>`
        : text(state.language, 'hint')
...
async function initialize() {
  ...
  if (!localStorage.getItem('atlas-language'))
    state.language = state.data.settings.language || 'zh-CN';

  const defaultEngine = Number(state.data.settings.default_search_engine);
  if (
    state.data.settings.default_search_engine !== 'local' &&
    state.data.searchEngines.some((engine) => engine.id === defaultEngine)
  )
    state.searchMode = defaultEngine;
  render();
}
...
$('#search-mode').addEventListener('click', (event) => {
  ...
  $('#search-menu').hidden = !$('#search-menu').hidden;
});
...
$('#layout-toggle').addEventListener('click', (event) => {
  event.stopPropagation();
  $('#layout-menu').hidden = !$('#layout-menu').hidden;
});
document.querySelectorAll('#layout-menu [data-layout]').forEach((button) =>
  button.addEventListener('click', () => {
    localStorage.setItem('atlas-layout', button.dataset.layout);
    render();
  }),
);
...
document.addEventListener('keydown', (event) => {
  ...
  if (state.searchMode === 'local') document.querySelector('.link-card')?.click();
    ...
    const engine = state.data.searchEngines.find(
      (item) => item.id === state.searchMode,
    );
    if (engine)
      window.open(
        engine.query_url.replace('{query}', encodeURIComponent(input.value.trim())),
        '_blank',
        'noopener',
      );
  }
...
$('#date-line').textContent = new Intl.DateTimeFormat('zh-CN', {
  weekday: 'long',
  month: 'long',
  day: 'numeric',
}).format(new Date());
initialize().catch((error) => {
  console.error(error);
  $('#catalog').innerHTML =
    '<p class="error-note">Unable to load navigation data. Please check the local server.</p>';
});

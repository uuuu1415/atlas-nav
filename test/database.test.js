...
test('database configuration validates every provider', () => {
  ...
  assert.deepEqual(
    ...
    { provider: 'postgresql', url: 'postgres://localhost/atlas' },
  );
  assert.deepEqual(
    databaseConfig({
      DB_PROVIDER: 'mysql',
      DATABASE_URL: 'mysql://localhost/atlas',
    }),
    { provider: 'mysql', url: 'mysql://localhost/atlas' },
  );
  assert.deepEqual(
    databaseConfig({
      DB_PROVIDER: 'mongodb',
  ...
  assert.throws(
    () => databaseConfig({ DB_PROVIDER: 'postgresql' }),
  ...
  assert.throws(
    () => databaseConfig({ DB_PROVIDER: 'oracle' }),
...
test('repository contract rejects incomplete adapters', () => {
  const repository = Object.fromEntries(
    repositoryMethods.map((method) => [method, () => {}]),
  );
...
test('setup configuration validates provider-specific fields', () => {
  assert.deepEqual(
    validateSetupInput({ provider: 'sqlite', sqlitePath: './data/custom.db' }),
    { DB_PROVIDER: 'sqlite', SQLITE_PATH: './data/custom.db' },
  );
  assert.deepEqual(
    validateSetupInput({
      provider: 'postgresql',
    ...
    {
      DB_PROVIDER: 'postgresql',
      DATABASE_URL: 'postgresql://atlas:secret@db.internal:5432/atlas',
    },
  );
  assert.throws(
    () =>
  ...
  assert.deepEqual(
    ...
    {
      DB_PROVIDER: 'mongodb',
      MONGODB_URI: 'mongodb://atlas:secret@localhost:27017',
      MONGODB_DATABASE: 'atlas',
    },
  );
  assert.throws(

...
export const setupConfigPath = path.resolve(
  root,
  process.env.ATLAS_CONFIG_PATH || '.atlas-nav.config.json',
);
const publicRoot = path.resolve(root, 'public') + path.sep;
if (setupConfigPath.startsWith(publicRoot))
  throw new Error('ATLAS_CONFIG_PATH must not be inside the public directory.');
...
export function readSetupConfig() {
  ...
  if (
    ...
    throw new Error('Atlas Nav configuration is invalid or unreadable.');
  return value;
}
...
export function hasDatabaseConfiguration() {
  ...
  if (provider === 'mongodb')
    return Boolean(
      process.env.MONGODB_URI?.trim() && process.env.MONGODB_DATABASE?.trim(),
    );
  return false;
}
  ...
  if (provider === 'sqlite') {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      if (!resolved.startsWith(dataRoot))
        throw new Error('生产环境 SQLite 文件必须位于项目 data 目录内。');
    }
    ...
    const uri =
      clean(body.uri) ||
      (mongoHost && Number.isInteger(mongoPort) && mongoPort > 0 && mongoPort < 65536
        ? `mongodb://${mongoUsername ? `${encodeURIComponent(mongoUsername)}:${encodeURIComponent(mongoPassword)}@` : ''}${mongoHost}:${mongoPort}`
        : '');
    if (!/^mongodb(?:\+srv)?:\/\/[^\s]+$/i.test(uri))
      throw new Error('MongoDB URI 无效。');
    ...
    if (!clean(body.username) || !clean(body.password) || !clean(body.database))
      throw new Error('请填写数据库用户名、密码和数据库名。');
  }
  return result;
}

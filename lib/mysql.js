...
export async function createMysql(config) {
  ...
  const query = async (sql, values = [], connection = pool) => { const [result] = await connection.execute(sql, values); return { rows: Array.isArray(result) ? result : [], insertId: result.insertId }; };
  const transaction = async fn => { const connection = await pool.getConnection(); try { await connection.beginTransaction(); const result = await fn((sql, values) => query(sql, values, connection)); await connection.commit(); return result; } catch (error) { await connection.rollback(); throw error; } finally { connection.release(); } };
  try { const repository = await createRelationalRepository({ dialect: 'mysql', query, transaction, schema }); repository.close = () => pool.end(); return repository; } catch (error) { await pool.end(); throw error; }
}

...
export async function createPostgresql(config) {
  ...
  const convert = sql => { let index = 0; return sql.replace(/\?/g, () => `$${++index}`).replace(/`key`/g, 'key'); };
  const query = async (sql, values = [], client = pool) => { const result = await client.query(convert(sql), values); return { rows: result.rows }; };
  const transaction = async fn => { const client = await pool.connect(); try { await client.query('BEGIN'); const result = await fn((sql, values) => query(sql, values, client)); await client.query('COMMIT'); return result; } catch (error) { await client.query('ROLLBACK'); throw error; } finally { client.release(); } };
  try { const repository = await createRelationalRepository({ dialect: 'postgresql', query, transaction, schema }); repository.close = () => pool.end(); return repository; } catch (error) { await pool.end(); throw error; }
}

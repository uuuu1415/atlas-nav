...
export function databaseConfig(env = process.env) {
  ...
  if (!providers.has(provider)) throw new Error(`DB_PROVIDER must be one of: ${[...providers].join(', ')}. Received: ${provider || '(empty)'}.`);
  if (provider === 'sqlite') return { provider, path: env.SQLITE_PATH?.trim() || './data/atlas-nav.db' };
  if (provider === 'postgresql') return { provider, url: connectionUrl(required(env, 'DATABASE_URL', provider), ['postgres:', 'postgresql:'], 'DATABASE_URL') };
  if (provider === 'mysql') return { provider, url: connectionUrl(required(env, 'DATABASE_URL', provider), ['mysql:'], 'DATABASE_URL') };
  return {
    provider,
    uri: connectionUrl(required(env, 'MONGODB_URI', provider), ['mongodb:', 'mongodb+srv:'], 'MONGODB_URI'),

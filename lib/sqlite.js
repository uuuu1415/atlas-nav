...
export const sqlite = {
  ...
  getSettings() {
    return Object.fromEntries(
      many('SELECT key, value FROM settings').map((row) => [row.key, row.value]),
    );
  },
  ...
  categories(includeHidden = false) {
    ...
    return many(
      `SELECT c.*, COUNT(l.id) AS link_count FROM categories c LEFT JOIN links l ON l.category_id = c.id ${where} GROUP BY c.id ORDER BY c.sort_order, c.id`,
    );
  },
  category(id) {
  ...
  deleteCategory(id) {
    transaction(() => {
      run('DELETE FROM links WHERE category_id = ?', id);
  ...
  reorderCategories(ids) {
    transaction(() =>
      ids.forEach((id, i) =>
        run('UPDATE categories SET sort_order = ? WHERE id = ?', i, id),
      ),
    );
  },
  links(categoryId = null, includeHidden = false) {
    const parts = [];
    const values = [];
    if (categoryId) {
      parts.push('l.category_id = ?');
      values.push(categoryId);
    }
    if (!includeHidden) parts.push('l.visible = 1 AND c.visible = 1');
    const where = parts.length ? `WHERE ${parts.join(' AND ')}` : '';
    return many(
      `SELECT l.*, c.name AS category_name FROM links l JOIN categories c ON c.id=l.category_id ${where} ORDER BY l.pinned DESC, l.sort_order, l.id`,
      ...values,
    );
  },
  link(id) {
  ...
  createLink(data) {
    ...
    const r = run(
      'INSERT INTO links (category_id,name,url,description,aliases,icon_type,icon_value,color,sort_order,pinned,visible) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      data.category_id,
      data.name,
      data.url,
      data.description,
  ...
  reorderLinks(ids) {
    transaction(() =>
      ids.forEach((id, i) =>
        run('UPDATE links SET sort_order = ? WHERE id = ?', i, id),
      ),
    );
  },
  searchEngines(includeHidden = false) {
    return many(
  ...
  createSearchEngine(data) {
    const sort = one(
      'SELECT COALESCE(MAX(sort_order), -1) + 1 AS next FROM search_engines',
    ).next;
    const r = run(
      'INSERT INTO search_engines (name, query_url, icon, color, sort_order, visible) VALUES (?, ?, ?, ?, ?, ?)',
      data.name,
      data.query_url,
      data.icon,
      data.color,
  ...
  deleteSearchEngine(id) {
    run('DELETE FROM search_engines WHERE id = ?', id);
  },
  reorderSearchEngines(ids) {
    transaction(() =>
      ids.forEach((id, i) =>
        run('UPDATE search_engines SET sort_order = ? WHERE id = ?', i, id),
      ),
    );
  },
  setLinkHealth(id, status) {
    run(
  ...
  allAdmins() {
    return many('SELECT id, username, password_hash, created_at FROM admins');
  },
  updateAdminPassword(username, passwordHash) {
    run('UPDATE admins SET password_hash=? WHERE username=?', passwordHash, username);
  },
  ...
  recentLoginAttempts(username, after) {
    return one(
  ...
  exportData() {
    return {
      ...
      categories: many('SELECT * FROM categories ORDER BY sort_order,id'),
      links: many('SELECT * FROM links ORDER BY sort_order,id'),
      searchEngines: many('SELECT * FROM search_engines ORDER BY sort_order,id'),
    };
  },
  importData(payload) {
    transaction(() => {
      ...
      run('DELETE FROM search_engines');
      run('DELETE FROM settings');
      for (const [key, value] of Object.entries(payload.settings || {}))
        this.setSetting(key, String(value));
      for (const c of payload.categories || [])
        run(
          'INSERT INTO categories (id,name,description,icon,color,sort_order,visible) VALUES (?,?,?,?,?,?,?)',
          c.id,
          c.name,
          c.description || '',
          c.icon || '◌',
      ...
      for (const l of payload.links || [])
        run(
          'INSERT INTO links (id,category_id,name,url,description,aliases,icon_type,icon_value,color,sort_order,pinned,visible,health_status,health_checked_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
          l.id,
          l.category_id,
          l.name,
          l.url,
      ...
      for (const e of payload.searchEngines || [])
        run(
          'INSERT INTO search_engines (id,name,query_url,icon,color,sort_order,visible) VALUES (?,?,?,?,?,?,?)',
          e.id,
          e.name,
          e.query_url,
          e.icon || '⌕',

...
网页更新默认启用，不需要在旧版 `.env` 中增加 `ATLAS_ALLOW_WEB_UPDATE=1`。如需禁用，可设置 `ATLAS_ALLOW_WEB_UPDATE=0`。安装器会设置 `TRUST_PROXY=loopback`：登录 Cookie 会在首次 HTTP 初始化时正常工作，部署 Nginx HTTPS 后则自动标记为 `Secure`；只有本机 Nginx 反代可传递受信任的 HTTPS 状态。后台“维护工具”中的“检查更新”和“更新”仍要求服务器是 Git 部署、工作区干净且远端 `main` 有新提交；更新会运行 `git pull --ff-only`、`npm ci --omit=dev`，随后由 systemd 自动重启服务。
